...
>
> **Era Material Design & Flat — ✅ implementata** (`src/lib/themes/Material.svelte`, chiave tema `'material'`, hash `#material`, in `ERA_ORDER` **tra `skeuo` e `parallax`**). La "grande appiattita" 2014–2017 (Google Material + iOS 7): **app bar estesa indaco** (elevazione 4dp) con avatar/nome/ruolo/tagline/place-chip + icon-button contatti, **tab strip sticky** con **ink-bar accent scorrevole** + **scroll-spy** (click→scroll fluido, scroll→tab sincronizzata), **card flat su `#fafafa` sollevate da ombre di elevazione** (token 2/4/6/8/12dp, hover→8dp), esperienze con avatar-monogramma colorato + highlight a spunta + tech-chip, competenze in **chip tonali**, lingue come **LinearProgress determinato**, **FAB accent** (6→12dp, azione *mailto*) e **ripple** autentico su tab/icon-button/contatti/FAB. Tipografia **Roboto** self-hostata (file variabile latin-subset in `fonts.css` + `/static/fonts/roboto.woff2`), cue audio `case 'material'` (tap sine pulito). Tutto gated da `prefers-reduced-motion`. **Differenziazione da Bento**: Material = superfici piatte + elevazione a ombre + ripple + ink-bar + accento forte (no vetro smerigliato / no bento-grid). Scheda completa in `ERE.md` §6.
>
> **Era Glassmorphism — ✅ implementata** (`src/lib/themes/Glass.svelte`, chiave tema `'glass'`, hash `#glass`, in `ERA_ORDER` **tra `threed` e `brutalism`**, label d'anno **"2020"**, icona 🧊). Il **presente luminoso** (macOS Big Sur / Windows 11 / iOS), differenziato dal 3D col principio **"Luce vs Buio"**: **nessun WebGL** — sfondo di **aurora-blobs pastello** (aqua/lilla/pesca/cielo/menta) su base chiara, sfocati e in **drift CSS lento** con **parallax 2D piatto** verso il cursore (no camera/prospettiva). **Vetro frosted presente e lattiginoso** (`rgba(255,255,255,.58)` chiaro / `rgba(28,28,44,.55)` scuro + `backdrop-filter: blur(30px) saturate(180%)`, bordo-luce 1px inset, ombra ambient), raggi **18px** sui pannelli / **12px** su `.exp-item`. Tipografia **Outfit ultralight** (geometrico arioso self-hostato `~34KB`, variable `wght 100–900`; ≠ Orbitron del 3D). Layout a pannelli fluttuanti (hero → Profilo → Esperienza → Competenze/Lingue → Formazione/Conferenze), contenuti da `cv-data.ts`. Cue audio `case 'glass'`: **campanella di vetro** cristallina (≠ drone scuro del 3D). Tutto gated da `prefers-reduced-motion`. **Nessuna interazione col cursore sul vetro** (rimossi `use:tilt` e lo sheen speculare, che erano diventati la firma di `liquid`): il vetro è CSS fermo, il pannello si solleva in hover ma non si inclina più. **Differenziazione da `liquid`**: Glass = documento (scroll unico, sfoca) senza navigazione; Liquid = app (shell a tab) che piega la luce con una lente. Scheda completa in `ERE.md` §9.
>
> **Era Liquid Glass — ✅ implementata** (`src/lib/themes/Liquid.svelte` + modulo puro `src/lib/themes/liquid/lens.ts`, chiave tema `'liquid'`, hash `#liquid`, **ultima** in `ERA_ORDER`, dopo `bento`, label d'anno **"2025"**, icona 💧). Colma il buco di navigazione che `Glass` (documento senza tab) lasciava aperto: **Glass = documento, Liquid = app**. **App shell** a `100dvh`: testata **sempre `position: absolute`** (mai `fixed`, per evitare il jank di iOS Safari con `backdrop-filter` su elementi `fixed`) con due barre impilate — striscia identità (`name · role`) + **tab bar a capsula** con **pillola indicatore a molla** (`cubic-bezier(0.34, 1.56, 0.64, 1)`, si deforma nella direzione del movimento) — sopra un'**unica regione scrollabile** con **quattro schermate a tab** (Profilo / Percorso / Competenze / Altro, ARIA `tablist`/`tab`/`tabpanel` completo, frecce ←/→ con wrap, `Home`/`End`) che coprono tutti i contenuti dei sette pannelli di Glass. La testata è una **capsula flottante centrata** (`min(468px, 100% - 24px)`, non una fascia a tutta larghezza) con **`pointer-events: none`** sul vetro, così la rotellina sopra la barra scorre comunque la pagina. Il contenuto **scorre dietro la testata** (scelta deliberata: dà alla lente qualcosa da rifrangere); allo scroll le due barre collassano in un'unica **capsula compatta** e restano tali finché si è lontani dalla cima (il vetro grande scompare: resta solo la pillola), riespandendosi al focus da tastiera / puntatore / tocco sulla pillola. Il cambio di tab riporta lo scroll in cima. Sotto i 720px la capsula scende a `top: 62px`, sotto la riga di chrome. **Ambiente**: tre wallpaper **CSS puro** (Aurora/Sunset/Deep, mesh gradient saturi, 0 asset), ciclati da un pulsante, persistiti in `localStorage` (`cv_liquid_wallpaper`), ognuno con la propria `--l-accent`; sostituisce, senza duplicarlo, il toggle chiaro/scuro di Glass. **La lente**: filtro SVG `feImage` + tre `feDisplacementMap` a `scale` diversa per R/G/B (iridescenza sul bordo), *displacement map* generata a runtime su `<canvas>` da `liquid/lens.ts` (testato con `vitest`) e rigenerata a ogni cambio di geometria via `ResizeObserver` (**cache per geometria + debounce**: le due forme si pagano una volta, non a ogni scroll), `id` del filtro **per istanza** (il cambio lingua incrocia due `Liquid`); vive solo sulla testata. **Detection**: `navigator.vendor === 'Google Inc.'` → lente; ogni altro motore (WebKit, incluso ogni browser iOS/iPadOS, e Gecko) → **fallback curato** (rim-light conico, doppia ombra inset ciano/magenta, anello — non pannello — con `backdrop-filter` leggero); default sempre a fallback. Tipografia **di sistema** (`-apple-system`/SF Pro/Segoe UI Variable), 0 webfont, pesi 500–600, tracking stretto — l'opposto voluto dell'Outfit ultralight di Glass. Cue audio `case 'liquid'`: **goccia** acquatica (`sine`, pitch che scende e risale di scatto). `prefers-reduced-motion`: tab sempre funzionanti, niente collasso/molla/transizioni tra schermate, lente attiva (è statica). **Contrasto misurato** sui tre wallpaper (tutto ≥ 4.5:1): il velo del vetro della testata **scurisce** (`rgba(14, 18, 38, 0.42)`) invece di schiarire, e le pillole della Timeline in quest'era sono **opache** (translucide lasciavano passare il tracciato attraverso l'etichetta). Scheda completa in `ERE.md` §16.
>
...
>
> **Era Parallax / Immersive Scroll — ✅ implementata** (`src/lib/themes/Parallax.svelte`, chiave tema `'parallax'`, hash `#parallax`, in `ERA_ORDER` **tra `material` e `threed`**, label d'anno **"2018"**, icona 🎬). Il **web premium "craft"** contemporaneo (stile *synthesis.capital* / *tresmarescapital.com*): il sito diventa un'**esperienza cinematografica da scrollare**, dove **lo scroll è il regista** e l'**estetica è la protagonista** (contenuti = sottoinsieme curato di `cv-data.ts`; formazione/conferenze/lingue restano nelle altre ere). **Smooth-scroll con inerzia** via **Lenis** (~3KB, lazy come Three, spento sotto reduced-motion); **parallax multi-piano** + **reveal coreografati** (nome/manifesto a **parole mascherate**, **righe esperienza che entrano alternando lato**, statement con keyword evidenziate in terracotta). **7 scene**: hero con **composizione astratta animata** (anelli orbitali, diamante, orb, dot, "+" su più piani), profilo-manifesto, percorso (index editoriale + hover micro-dettaglio), **numeri in cifre** (sezione *pinned* con **contatori animati** da `cv-data.ts`: anni dal 2011/prodotti/settori, da `keyFigures`), competenze (**marquee cinetico**), ritratto (avatar **duotone**), CTA (bottoni **magnetici** + contatti reali). **Cursore custom**, barra di **progresso**, **indice a puntini** cliccabile, **auto-scroll "▶ Play"** opzionale. Serif display **Fraunces** self-hostato/subsettato (`fonts.css` + `/static/fonts/fraunces.woff2`, ~67KB, variabile opsz/wght; corpo in **Inter**). Cue audio `case 'parallax'` (**swell aereo ascendente**, ≠ campanella Glass / drone 3D). **Doppio binario** `prefers-reduced-motion`: scroll verticale statico, marquee → **lista skill accessibile**, niente parallax/auto-scroll/cursore — contenuti sempre integri. Scheda completa in `ERE.md` §15.
>
> **Cronologia rivista (set 2026)** — `ERA_ORDER` = `terminal · teletext · pixel · web1 · y2k · winxp · skeuo · material · parallax · threed · glass · brutalism · bento · liquid`. Il Brutalismo implementato è **neubrutalism** (bordi spessi, ombre offset dure, font display): label **"Neubrutalism · 2021"**, dopo `glass`. Il Bento è **"Bento · 2022"** (non più "Modern Flat · 2015"). Il 3D è **"WebGL · 2019"** (i portfolio WebGL immersivi da Awwwards), non più "Future 3D · 2026": sta **tra `parallax` e `glass`**, e la timeline oggi chiude con `liquid`. Analisi completa in `ANALISI-STILI.md`.
>
> **Era Y2K — ✅ implementata** (`src/lib/themes/Y2K.svelte`, chiave `'y2k'`, hash `#y2k`, in `ERA_ORDER` **tra `web1` e `winxp`**, label **"Y2K · 2000"**, icona 💿). Web Flash del 2000: intro con logo cromato e `skip intro`, console 960×600 in cromo e plastica Bondi con pulsanti gel (tablist), schermo CRT con transizioni da Flash, orologio "Y2K COMPLIANT ✓". Michroma self-hostata, corpo Verdana/Tahoma. **Skin della Timeline via token `--tl-*`**: le ere nuove definiscono solo i token in un blocco `:global(:root) .theme-X`, senza nuovi blocchi per elemento. Scheda in `ERE.md` §12.
>
> **Prossima era proposta — AI / Conversazionale** (⬜ §11): l'unica vera nuova frontiera narrativa oltre il 3D (agente deterministico `cv-data.ts`-driven). Andrebbe **in coda a `ERA_ORDER`**, dopo `liquid`, come nuovo finale. Scheda in `ERE.md` §11.
...
- **Isolamento per-tema**: ogni era è un componente Svelte autonomo in `src/lib/themes/` (`Terminal`, `Teletext`, `PixelArt`, `Web1`, `Y2K`, `WinXP`, `Skeuo`, `Material`, `Brutalism`, `BentoBox`, `Parallax`, `Glass`, `Liquid`, `ThreeD`), con logica e CSS propri. La Timeline (`src/lib/components/Timeline.svelte`) inietta stile globale per-tema.
- **Stato tema**: `src/lib/store.ts` (`currentTheme`) persiste in `localStorage` (`cv_theme`) ed è sincronizzato con l'hash dell'URL (deep-link `#terminal`/`#teletext`/`#pixel`/`#web1`/`#y2k`/`#winxp`/`#skeuo`/`#material`/`#brutalism`/`#bento`/`#parallax`/`#glass`/`#liquid`/`#threed`). `ERA_ORDER` definisce l'ordine cronologico (per la transizione direzionale). Il primo paint è gated da un flag `booted` in `+page.svelte`: finché l'era reale non è risolta dall'hash/`localStorage` non si renderizza nulla, così non si vede il flash del tema di default né parte l'FX al boot — solo un fade-in pulito.
