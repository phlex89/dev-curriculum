...
-   **Estetica:** **8 colori puri** su nero (il blu è alzato a `#6c7bff` per leggibilità), **titoli a doppia altezza**, **barra rainbow** a mosaico come separatore. Tipografia **Bedstead** (ricostruzione del SAA5050 di Ceefax/Televideo, pubblico dominio, self-hostata ~7KB; fallback JetBrains Mono). Cue audio `case 'teletext'` (doppio blip secco, distinto dal beep CRT del Terminale).
...
-   **Mini-gioco overworld top-down** in stile RPG 8-bit (Zelda/Pokémon, **non** un platformer): si cammina su una mappa a tile vista dall'alto (48×32, generata in `buildGrid()`, densità pixel `--px = tile/32`; eroe/edifici/monete a dimensione assoluta via override `--px = tile/16`, collisioni/interazioni a blocco 2×2), con **camera che segue il personaggio**. Lineage parallela al Terminale (mondo console, non desktop), per questo in `ERA_ORDER` è subito **dopo `terminal`**.
