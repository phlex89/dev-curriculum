...
>
> **Cronologia rivista (set 2026)** — `ERA_ORDER` = `terminal · teletext · pixel · web1 · winxp · skeuo · material · parallax · threed · glass · brutalism · bento · liquid`. Il Brutalismo implementato è **neubrutalism** (bordi spessi, ombre offset dure, font display): label **"Neubrutalism · 2021"**, dopo `glass`. Il Bento è **"Bento · 2022"** (non più "Modern Flat · 2015"). Il 3D è **"WebGL · 2019"** (i portfolio WebGL immersivi da Awwwards), non più "Future 3D · 2026": sta **tra `parallax` e `glass`**, e la timeline oggi chiude con `liquid`. Analisi completa in `ANALISI-STILI.md`.
>
...
- **Isolamento per-tema**: ogni era è un componente Svelte autonomo in `src/lib/themes/` (`Terminal`, `Teletext`, `PixelArt`, `Web1`, `WinXP`, `Skeuo`, `Material`, `Brutalism`, `BentoBox`, `Parallax`, `Glass`, `Liquid`, `ThreeD`), con logica e CSS propri. La Timeline (`src/lib/components/Timeline.svelte`) inietta stile globale per-tema.
- **Stato tema**: `src/lib/store.ts` (`currentTheme`) persiste in `localStorage` (`cv_theme`) ed è sincronizzato con l'hash dell'URL (deep-link `#terminal`/`#teletext`/`#pixel`/`#web1`/`#winxp`/`#skeuo`/`#material`/`#brutalism`/`#bento`/`#parallax`/`#glass`/`#liquid`/`#threed`). `ERA_ORDER` definisce l'ordine cronologico (per la transizione direzionale). Il primo paint è gated da un flag `booted` in `+page.svelte`: finché l'era reale non è risolta dall'hash/`localStorage` non si renderizza nulla, così non si vede il flash del tema di default né parte l'FX al boot — solo un fade-in pulito.
